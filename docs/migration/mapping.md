...
| 采集 | `novel_crawl_*`、历史 `crawl_*` | PostgreSQL 内容生产域 | 原值保留 | 来源、游标、候选、任务与日志关联 | 运行时目标表已建立，但旧数据尚无迁移 stage；历史表启用状态、Cookie Secret 转换、自动发现运行状态和 legacy 幂等键待按 `docs/verification/m5-m6-content-migration-audit.md` 关闭 |
