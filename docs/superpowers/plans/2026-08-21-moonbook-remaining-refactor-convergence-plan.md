...
- [x] **Step 1：建立覆盖清单测试。** 每个注册 migration stage 必须有行数、主键、关联和业务核对项；测试在增加新 stage 而未增加核对时失败。
- [x] **Step 2：补内容生产核对。** 覆盖论坛来源/板块/候选/任务/日志、TXT、AI 配置/模型、清洗、摘要、画像和合并血缘。
- [x] **Step 3：补对象核对。** 从 PostgreSQL 业务引用流式读取对象，逐对象 `Stat/Get` 校验存在性、kind、owner、字节和 SHA-256；报告聚合和最多 100 个哈希指纹，不输出对象键或正文。
- [x] **Step 4：扩展全域财务核对。** 保持钱包、流水、订单、支付、回调、会员和权益零差异门，异常退出非零。
- [x] **Step 5：真实双库与 MinIO 测试。** 注入行数、关联、财务、缺对象、错大小和错哈希故障，证明每类差异可被稳定分类且无静默通过。
- [x] **Step 6：提交。** 提交信息：`完善全域迁移与对象核对`。

验收记录见 `docs/verification/m6-global-migration-object-audit.md`。
...
- [x] **Step 1：删除陈旧事实。** 修正已完成运营页签、财务全域核对、内容迁移 stage、自动发现调度和模块边界整改仍被写成未完成的问题。
- [x] **Step 2：逐项链接证据。** 每个矩阵能力链接代码、自动测试、运行报告、提交或批准移除记录。
- [x] **Step 3：更新 R1-R4 状态。** 只有直接证据通过才勾选；真实第三方和商业授权进入外部清单，不混为代码缺口。
- [x] **Step 4：提交。** 提交信息：`更新剩余重构验收状态`。

Task 12 已把 M4/M5 的本地完成项与外部验收分离，M6 明确为“核对器完成、TXT 输入阻断”，M7 原审计中的监控、安全、统一 CI、维护/部署合同等陈旧缺口已改为直接证据。管理矩阵不再保留无解释的“基线已盘点”。
