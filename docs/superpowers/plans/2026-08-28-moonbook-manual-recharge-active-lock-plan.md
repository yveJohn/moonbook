...
- [ ] 扩展真实 PostgreSQL 人工补单测试，先证明成功补单后 `active_reader_id` 残留。
- [ ] 在补单结算事务中将 `active_reader_id` 置空。
- [ ] 验证首次补单与幂等重放都保持 `paid`、单钱包流水和空活动锁。

### 任务二：建单自愈历史终态锁

- [ ] 扩展真实 PostgreSQL 充值仓储测试，构造带活动锁的 `paid` 历史订单并证明建单失败。
- [ ] 在读者 advisory lock 事务内清除明确终态的遗留活动锁，再继续创建新订单。
- [ ] 自愈更新带订单、读者和终态条件；未知状态继续拒绝。
- [ ] 验证旧订单支付事实不变，新订单创建成功，`pending` 替换及不确定订单保护不变。

### 任务三：验证、提交与发布

- [ ] 执行相关 Go 单元测试、真实 PostgreSQL 集成测试、`go vet` 和 Commerce 全包回归。
- [ ] 更新 `docs/progress/refactor-status.md`，记录生产根因、实现和验证证据。
- [ ] 检查全部修改、敏感信息、`git diff --check` 和暂存区，使用简体中文逻辑提交。
