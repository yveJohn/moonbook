...
- [x] 只修改活跃仓库，不修改冻结仓库或 `reader-ui` 业务代码。
- [x] 不连接生产、不使用真实支付凭据、不执行真实支付、正式迁移、切流或 `git push`。
- [x] 每个任务先写失败测试并确认失败原因，再做最小完整实现；不得删除、跳过或放宽既有测试。
- [x] PostgreSQL 迁移只新增，不修改、改名或删除 `00001` 至 `00059`；创建前必须重新检查最高版本。
- [x] Go 测试使用本机 JDK 无关；Go 命令在 `server` 下执行，使用项目现有 Go 工具链和隔离 `GOCACHE`。
- [x] 真实集成测试只使用项目专用 PostgreSQL，不使用 SQLite；本地 HTTP 替身只监听回环地址。
- [x] Secret 不进入源码、数据库、日志、快照或测试报告；`.env.example` 只含空值或明确非秘密示例。
- [x] Long ID 继续在 Reader JSON 和管理前端按字符串处理。
- [x] 每个逻辑单元验证后检查全部 tracked/untracked 修改，使用简体中文独立提交。
- [x] 本切片完成不等于 M4 完成，回调全尝试审计、运营审计页签、全域核对和真实支付外部验收继续保留。
...
- [x] **Step 1：迁移验证。** 空库、`00059 -> 00060`、重复执行和关键行数保护全部通过。
- [x] **Step 2：后端质量门。** 定向测试、完整 Commerce/Reader 模块测试、race、vet、模块依赖静态门通过。
- [x] **Step 3：冻结 Reader 门。** 源树差异仍为零，536 个测试、生产构建和充值契约通过。
- [x] **Step 4：Compose 门。** 配置合同、Compose 模型、Shell 语法、隔离栈创建/查询/管理可见性和秘密扫描通过。
- [x] **Step 5：记录固定证据。** 写入 commit、工具版本、命令、迁移结果、网关替身场景、并发次数和剩余外部项。
- [x] **Step 6：更新结论。** 只关闭支付创建、替换、失败和过期缺口；回调全尝试审计、运营页签、全域核对与真实支付继续未完成，M4 保持实施中。
- [x] **Step 7：最终检查。** 检查全部 tracked/untracked、忽略产物、秘密、`git diff --check` 和暂存区。
- [x] **Step 8：提交。** 提交信息：`补充EPUSDT支付创建验收证据`。
- [x] **Step 9：状态确认。** `git status` 干净且未 push，继续下一 M4 退出项。
