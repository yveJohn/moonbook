...
- [ ] **Step 1：补失败测试。** 固定权益、会员商品、邀请面板、书架、点赞、历史的排序、空值、日期、已发布过滤和 Long ID 字符串。
- [ ] **Step 2：账号商业视图改用 Commerce 合同。** Reader 不查询权益、会员、商品、奖励配置或钱包流水表。
- [ ] **Step 3：个人内容使用 Novel 批量快照。** Reader 先读自身书架/历史/点赞事实，去重收集 book/chapter ID，一次批量调用后稳定组装，缺失或未发布目标按冻结语义处理。
- [ ] **Step 4：证明无 N+1。** 使用计数替身和真实数据库查询统计，断言每页最多固定次数合同调用。
- [ ] **Step 5：验证并提交。** 提交信息：`重构读者跨域查询投影`。
