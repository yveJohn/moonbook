...
- Reader 逐接口矩阵开始按路由组收敛：个人数据的书架、点赞、反馈、历史、偏好共 13 条路由已完成正常 JSON 快照，覆盖显式 `data:null`、空数组/分页归一化和两个 Long ID 边界；认证契约补齐过期、撤销、封禁账号的统一业务 `401`。公开接口仍有精选/随机/分类/SEO 边界，Commerce 仍有钱包/充值/购买 HTTP 快照待补，不能据此关闭 M3。
