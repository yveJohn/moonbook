...
- 人工补单后第二次充值返回 `internal service error` 的根因已在生产只读确认：订单 `RC2089645633658621956` 已为 `paid`，钱包流水与 `manual_success` 审计存在，但人工补单事务未清除 `active_reader_id`，后续建单因遇到意外的活动 `paid` 状态而失败。提交 `a4948f8` 已让人工补单在同一事务释放活动锁，并让充值建单在读者 advisory lock 内对遗留的 `paid`、`create_failed`、`expired`、`superseded` 活动锁执行带订单、读者和状态条件的精确自愈；`creating`、`gateway_unknown`、`pending` 与未知状态边界保持不变，自愈不修改旧订单状态、交易号、实付金额、钱包和审计事实。未直接更新生产订单或财务数据。Commerce 全包测试、定向 `go vet`、两个受影响包的单元测试及真实 PostgreSQL 全部集成测试通过；生产脚本发布与线上复验待执行。
