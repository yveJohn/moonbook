...
- Reader 充值订单页金额展示已统一为四位小数：提交 `5227a99` 使用十进制字符串格式化，不经 JavaScript `Number`；顶部“需要支付”继续优先使用网关 `actualAmount`，明细“基础定价”使用 `priceUsdt`，`2.00000000`、`2.58` 分别显示为 `2.0000`、`2.5800`。异常的非零高精度值保留原文，不静默改变应付金额。Reader 定向 16 项测试、全量 45 个测试文件/542 项用例和生产构建通过；生产发布与线上复验待执行。
