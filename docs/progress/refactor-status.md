...
| M5 内容生产与长任务 | 实施中 | 论坛来源、板块、候选、导入任务、抓取日志和 TXT 上传/预览/导入队列已完成，迁移版本 `00043`；失败文件修复、AI、分页抓取和全量恢复演练仍待完成 |
...
- 新增 `novel_txt_import_task`（迁移 `00042`）、`/novel/txtImports` 管理 API/页面和独立 `txt_import` 持久化 Worker；上传先写入 MinIO 并校验字节数/SHA-256，再建立 PostgreSQL 任务和平台队列，Worker 重启可从租约恢复并使用 `txt_import` 章节来源写入目标书籍。`GET /novel/txtImports/:id/preview` 仅返回前 20 章和每章前 2000 个字符；前向迁移 `00043` 幂等补齐早期已执行 `00042` 环境的预览权限。模块单元测试、迁移嵌入清单测试和前端构建通过；失败文件修复、AI 任务、论坛多页抓取和全量重启演练仍待完成。
