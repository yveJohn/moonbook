...
- M7 模块边界整改的架构、合同、上下文事务、批量投影、错误和测试设计已逐节获用户确认；针对订单、充值订单和钱包管理搜索的准确筛选/计数/分页缺口，用户追加批准 Commerce 自有最小 Reader 搜索投影方案，专项书面规格已完成对应修订并等待复核。投影只用于 Commerce 管理搜索与展示，Reader 仍是事实源，鉴权和交易判断必须走实时合同；实施不得弱化冻结 Reader 契约、跨域原子性或现有边界测试，详见 `docs/superpowers/specs/2026-08-15-moonbook-module-boundary-remediation-design.md`。
