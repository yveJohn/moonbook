...
| M5 内容生产与长任务 | 实施中 | 论坛来源配置已完成，迁移版本 `00036`；板块、候选/导入任务、TXT、AI 和长任务恢复仍待完成 |
| M6 全量迁移与校验 | 未开始 | - |
| M7 生产切换就绪验收 | 未开始 | - |
...
- 新增抓取日志查询 API 和页面（`GET /novel/crawl/fetchLogs`、`GET /novel/crawl/fetchLogs/:id`），迁移 `00040` 建立请求/解析/导入阶段日志事实表、状态约束和任务索引；导入 worker 核心支持通过 `platform_jobs` claim/lease 执行、成功/失败任务状态联动及日志写入。新增受控 `HTTPExecutor`，限制 HTTP/HTTPS URL、超时、User-Agent、响应体大小和可重试状态码，使用本地 `httptest` 替身验证成功、服务端错误和大小限制；章节解析和 MinIO 正文导入仍待后续切片。
