...
- Reader 认证注册、登录、退出、资料和修改密码五条路由的正常 HTTP 结构已全部固定，注册/登录使用冻结 UI 的嵌套 `reader`，退出与改密显式返回 `data:null`；两个 Long ID 边界、BCrypt 改密、会话撤销及真实 PostgreSQL/Redis 回归均通过。管理员 Token 隔离和参数错误响应仍待补。
...
1. 固定 Reader 参数错误、管理员 Token 隔离、收费权益和余额不足等 HTTP 错误响应。
...
- 新增 `POST /novel/txtImports/:id/file`（迁移 `00044`）失败文件修复入口和管理页面操作；仅 `failed/cancelled` 任务允许替换，修复文件先经 MinIO 大小/SHA-256 校验，再在同一 PostgreSQL 事务中替换引用、清空质量/失败统计、重置任务与平台 Job 尝试次数并重新排队。AI 任务、论坛多页抓取和全量重启演练仍待完成。
