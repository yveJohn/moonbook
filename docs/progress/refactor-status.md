...
- 新增抓取日志查询 API 和页面（`GET /novel/crawl/fetchLogs`、`GET /novel/crawl/fetchLogs/:id`），迁移 `00040` 建立请求/解析/导入阶段日志事实表、状态约束和任务索引；导入 worker 核心支持通过 `platform_jobs` claim/lease 执行、成功/失败任务状态联动及日志写入，执行器采用可注入接口，真实论坛网络抓取适配器仍待后续切片。
