...
- 完成 M5/M6 内容生产迁移审计：`all` 编排已覆盖论坛、TXT、合并和 AI stage，目标表已具备统一 legacy 来源键；旧清洗正文、TXT 文件和合并对象引用经 MinIO 大小/SHA-256 校验转换。审计确认论坛 Cookie 明文落库违反 Secret 注入约束；自动发现调度器已补齐，但真实论坛沙箱、Cookie Secret 改造、逐对象核对和 8GB 副本演练仍待完成。M5/M6 保持实施中，详见 `docs/verification/m5-m6-content-migration-audit.md`。
