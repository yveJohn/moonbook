...
- 管理端充值订单页展示已优化：提交 `2944db6` 将默认分页从 20 条调整为 10 条，状态筛选、列表和详情统一显示八种中文状态但继续用英文枚举查询和判断，列表创建时间复用管理前端日期工具显示为 `yyyy-MM-dd HH:mm:ss`，未知状态和空值均有明确回退。充值订单定向 2 项测试、管理前端完整 40 项测试、ESLint、生产构建和供应链检查通过；构建仅有既有 Vite 配置、分包大小和 `arcdash` BigInt 目标警告。变更只影响管理前端 Web，尚未执行部署。
