...
- Reader 邀请奖励明细缺失已关闭：旧奖励事实迁移映射原本存在，实际缺陷是 Reader HTTP 固定返回空数组、汇总读取错误配置/钱包流水，以及新 Go 注册发奖未写奖励事实。迁移 `00076` 增加 `(invitee_reader_id,reward_stage)` 唯一约束并只补建可由旧运行期不可变流水确定证明的注册事实；新注册在同一事务写 `register` 事实、邀请人 `invite_register_reward` 流水和被邀请人赠币。`POST /reader/me/invite/code` 现按事实表返回最近 20 条 `granted` 明细和全量累计，Long ID/金额为字符串、空时间为 `null`。quality、migration、真实 PostgreSQL/Redis/MinIO integration、Reader 542+6 项测试与生产构建通过；隔离 Playwright 注册两名读者后确认“已邀请 1/累计 13/邀请注册奖励 +13金币”，临时账号、奖励、流水、对象和数据库已清理。浏览器 fixture cleanup 已在受控事务内临时关闭并恢复钱包流水删除触发器，真实 PostgreSQL 验证账号、奖励、流水均为 0 且触发器恢复为启用。Reader 校验基线同步到已批准并部署的四位小数修复 `5227a99`，本次未修改 `reader-ui` 业务文件。
- 邀请奖励明细修复已于 2026-09-01 发布生产固定提交 `0b6b6e003b42`。发布前只读核对重复事实、可补建缺口和异常运行期邀请流水均为 0，生产奖励事实为 66 条；PostgreSQL 自定义格式备份保存于 `/opt/1panel/backup/moonbook/manual/invite-reward-20260901T065502Z`，SHA-256 与 `pg_restore --list` 校验通过。发布归档远端 SHA-256 校验通过，迁移由 75 前进至 76、`pending=0`，迁移后奖励事实仍为 66 条、重复事实和缺失运行期事实均为 0，唯一索引存在，未补写奖励或修改钱包余额。Server、Web、Reader 和 Gateway 已重建为 `0b6b6e003b42` 并全部 healthy；PostgreSQL、Redis 未重启且保持 healthy，R2 未执行对象写入。管理站、Reader 站、内部与公网健康端点均返回 200，readiness 四项依赖为 `ok`，四个应用容器发布后严重日志计数均为 0。未使用真实账号制造邀请或奖励数据。
- 本次发布后的额外只读财务核对发现 59 条既有成功购买订单缺少有效权益；迁移 `00076` 不涉及购买订单或权益，且生产邀请奖励事实迁移前后均为 66 条，因此该差异不属于本次邀请奖励发布回归。未对生产购买订单、权益、钱包或流水执行修数，需另立财务问题按受控方案排查。
- GM Pay v2 充值二维码不显示的根因已关闭：生产只读排查确认 GM Pay 已创建订单和收款地址，但 v2.0.0 成功响应固定返回大写 `currency=USD`、`token=USDT`，Moonbook 旧客户端按小写严格比较后把确定的成功响应误判为 `RESPONSE_MISMATCH`，订单进入 `gateway_unknown` 且未保存支付地址。客户端现仅对语义固定的 USD/USDT 使用大小写不敏感比较并规范化为小写，订单号、金额、实际金额、状态、地址、有效期和支付 URL 继续严格校验；各字段失败使用稳定脱敏码。过期逻辑保留已有创建诊断，管理订单详情增加失败码和失败原因。生产样本订单 `RC2089645633658621954`、`RC2089645633658621955` 已在 GM Pay 过期，不得支付。本地 EPUSDT/充值/管理订单测试、定向 `go vet`、迁移 75 的真实 PostgreSQL 集成测试、管理前端 39 项合同测试、ESLint 和生产构建通过。提交 `09639f10668d` 已通过本地构建远程发布脚本部署到生产：迁移无新增应用且最终为 `current=75 target=75 pending=false`，Server、Web、Reader 和 Gateway 均运行新镜像、healthy、零重启，管理站、Reader 站和 Reader API 公网 HTTPS 均返回 200，发布后 10 分钟 Server 无 error/fatal/panic 日志。
- 人工补单后第二次充值返回 `internal service error` 的根因已关闭：订单 `RC2089645633658621956` 已为 `paid`，钱包流水与 `manual_success` 审计存在，但人工补单事务未清除 `active_reader_id`，后续建单因遇到意外的活动 `paid` 状态而失败。提交 `a4948f8` 已让人工补单在同一事务释放活动锁，并让充值建单在读者 advisory lock 内对遗留的 `paid`、`create_failed`、`expired`、`superseded` 活动锁执行带订单、读者和状态条件的精确自愈；`creating`、`gateway_unknown`、`pending` 与未知状态边界保持不变，自愈不修改旧订单状态、交易号、实付金额、钱包和审计事实。未直接更新生产订单或财务数据。Commerce 全包测试、定向 `go vet`、两个受影响包的单元测试及真实 PostgreSQL 全部集成测试通过。固定候选 `0c76f1a9eb8f` 已通过本地构建远程脚本发布：迁移 `current=75 target=75 pending=false`，Server、Web、Reader 和 Gateway 均使用新镜像、healthy、零重启，管理站、Reader 站和就绪探针公网 HTTPS 均返回 200，发布后 Server 的 error/fatal/panic 精确计数为 0。生产遗留行未被直接改写，将在该读者下一次建单时由事务自愈。
- Reader 充值订单页金额展示已统一为四位小数：提交 `5227a99` 使用十进制字符串格式化，不经 JavaScript `Number`；顶部“需要支付”继续优先使用网关 `actualAmount`，明细“基础定价”使用 `priceUsdt`，`2.00000000`、`2.58` 分别显示为 `2.0000`、`2.5800`。异常的非零高精度值保留原文，不静默改变应付金额。Reader 定向 16 项测试、全量 45 个测试文件/542 项用例和生产构建通过。固定候选 `f427602c75a3` 已通过本地构建远程脚本发布：迁移 `current=75 target=75 pending=false`，Server、Web、Reader 和 Gateway 均使用新镜像、healthy、零重启，管理站、Reader 站和就绪探针公网 HTTPS 均返回 200，发布后 Server 的 error/fatal/panic 精确计数为 0。
- 管理端充值订单页展示已优化：提交 `2944db6` 将默认分页从 20 条调整为 10 条，状态筛选、列表和详情统一显示八种中文状态但继续用英文枚举查询和判断，列表创建时间复用管理前端日期工具显示为 `yyyy-MM-dd HH:mm:ss`，未知状态和空值均有明确回退。充值订单定向 2 项测试、管理前端完整 40 项测试、ESLint、生产构建和供应链检查通过；构建仅有既有 Vite 配置、分包大小和 `arcdash` BigInt 目标警告。固定候选 `394825fc697a` 已通过本地构建远程发布流程部署：归档上传及 SHA-256 校验成功，首次 SSH 在远程 `docker load` 完成后超时，检查确认三张新镜像完整、旧容器仍 healthy 且临时目录已清理，随后从 `prepare` 安全边界继续；迁移 `current=75 target=75 pending=false`，Server、Web、Reader 和 Gateway 均运行新镜像、healthy、零重启，生产 Web 产物包含本次中文状态，管理站、Reader 站及三个健康端点公网 HTTPS 均返回 200，发布后 Server 精确 `error/fatal/panic` 计数为 0。
...
- Reader 购买余额误判已关闭：普通计价章节、固定价章节和整书恢复旧系统的金币优先、钻石补足语义，会员固定只使用钻石，不再受商品 `allow_bonus_coin` 错误配置影响；商品迁移重跑会同步抵扣、有效期、排序及来源字段。Commerce 全包测试、定向 `go vet`、真实 PostgreSQL 购买测试以及只读 MySQL 8.4 到 PostgreSQL 17 的迁移重跑测试通过。
...
- M7 模块边界整改 Task 4 已把 Reader 注册、管理端账号状态和旧账号迁移接入 Commerce 搜索投影：注册账号、邀请码、邀请关系、双方奖励和投影在同一 PostgreSQL 事务内提交，状态变更与会话撤销、投影同步同成同败，session/token 仅在注册事务提交后创建；密码重置不刷新投影。迁移 Runner 将每批事务绑定到 Context，账号、投影与检查点可共同回滚并支持幂等重跑。新增双主键游标流式一致性服务，审计 missing/extra/mismatch，显式 repair 可收敛且只输出最多 100 个 Reader ID 哈希指纹。定向单元测试、`go vet`、真实 PostgreSQL 注册/状态/repair 集成测试及 MySQL 到 PostgreSQL 迁移回滚与重跑验证通过；验收库四字段双向差异为 0，钱包不可变触发器保持启用。边界违规由 15 项降为 14 项，余项按 Task 5 至 Task 8 继续整改。
- M7 模块边界整改 Task 5 已将 Reader-facing 签到、钱包、购买和充值 HTTP 编排迁入 `reader/commercecompat`，Commerce 通过窄合同 Provider 保留原 Service、Repository 和交易规则。11 条冻结路由的方法、路径、公开/认证边界均保持不变，HTTP 200 包装、业务 code/message、`data/rows/total` 空值、日期及 Long ID 字符串由 Reader 合同测试逐项锁定；新位置保留原 6 个契约测试入口并新增路由唯一性与认证清单测试。Reader/Commerce 全包测试、`go vet` 及真实 PostgreSQL 签到、购买、充值集成测试通过；模块边界违规由 14 项降为 7 项，剩余项属于 Task 6 和 Task 7 的 Reader public/account 整改。
- M7 模块边界整改 Task 6 已将 Reader public 改为只依赖 Novel 公开读取/SEO 合同和 Commerce 批量访问合同：已发布书籍、启用章节、分类、导航、SEO/sitemap 及 MinIO 正文查询全部归 Novel Provider，Reader 仅保留点赞、书架和阅读历史事实，并通过 Novel 合同补齐历史章节显示；付费判断先于正文对象读取。Novel 合同补充读取前章节元数据和内容版本，objectstore 以结构化错误区分对象缺失、依赖不可用、大小/SHA-256 完整性失败和 UTF-8 编码失败，HTTP 继续脱敏。Reader/Novel/Commerce 全包测试、`go vet`、合同替身测试及真实 PostgreSQL+Redis+MinIO 公开读取集成测试通过，冻结 Long ID、空值、日期、错误码和路由响应保持兼容；模块边界违规由 7 项降为 1 项，剩余 `reader/account` 归 Task 7 整改。
- M7 模块边界整改 Task 7 已将 Reader 账号商业视图改为只依赖 Commerce 账号汇总与邀请奖励合同，将书架、点赞和阅读历史改为先读取 Reader 自有事实，再通过 Novel 批量显示合同去重补齐书籍与章节信息。未发布、软删除、缺失及书籍章节不匹配目标按冻结语义过滤，Long ID、空值、日期、排序和 HTTP JSON 快照保持兼容；250 条结果仍只调用一次显示合同。定向测试、`go vet`、模块依赖规则及真实 PostgreSQL 的 Commerce 汇总、Novel 批量顺序/状态和 Reader 隔离集成测试通过，跨模块实现包 import 违规清零；Reader 中剩余 Novel SQL 仅为 Task 9 计划整改的 Like/Unlike 跨域原子事务。
- M7 模块边界整改 Task 8 已将 Reader 邀请注册拆为锁定邀请码、创建账号、邀请码计数、自动码、邀请关系和投影同步等独立事务参与步骤，再通过 Commerce 合同发放双方奖励；所有事实共用 Platform Context 事务，session/token 仅在提交后创建。注册与邀请码 ID advisory lock 先于 Reader 行锁，奖励按关系幂等锁及 Reader ID 稳定顺序处理钱包，钱包在读取幂等流水前取得事务锁。七阶段故障矩阵均证明零部分事实；真实 PostgreSQL 下同一限用邀请码 8 并发仅成功 1 次，同一奖励 8 并发仅产生双方各 1 条流水，受邀人流水冲突会回滚已写的邀请人奖励，嵌套错误即使被误吞仍由 rollback-only 阻止提交。定向测试、`go vet`、模块依赖规则和测试数据清理检查通过。
- M7 模块边界整改 Task 9 已将点赞改为 Reader Service 编排的同一 Context 事务：Novel Provider 先取得书籍级事务 advisory lock 并锁定已发布书籍，Reader Repository 仅幂等维护 `reader_book_likes` 并计算精确关系数，Novel Provider 再更新 `novel_books.like_count`。Reader 已移除全部 Novel 表 SQL，冻结 HTTP JSON、Long ID 字符串和错误语义保持不变。真实 PostgreSQL 覆盖缺失/未发布书籍、重复点赞/取消、Reader 关系失败、Novel 汇总失败及嵌套错误被吞后的 rollback-only；8 个读者并发点赞后关系数和汇总均为 8，并发取消后均为 0。定向测试、`go vet`、模块依赖规则和所有模块宽范围测试通过。
...
- 新增消费订单管理 API 和页面（`GET /reader/orders`、`GET /reader/orders/:id`），提供订单号/读者、订单类型、状态筛选及扣款拆分只读核对；所有 Long ID/金额字符串化，真实 PostgreSQL 集成测试覆盖列表、详情和读者关联，迁移版本推进至 `00032`。该只读切片当时未开放退款和人工充值；模拟充值已在后续 `00056` 两阶段闭环中补齐，退款仍需独立财务审批设计。
...
- 新增独立旧数据阶段 `reader-finance` 和前向迁移 `00054`，按确定依赖顺序迁移钱包、不可变流水、奖励桶、购买订单、签到规则/记录、邀请奖励、人工调账、充值配置、充值订单及回调日志。旧 bigint ID 原值保留并同步序列；商户 PID、回调来源 IP 只保存 SHA-256，旧支付凭据 Secret 不迁移。隔离只读 MySQL 8.4 到真实 PostgreSQL 17 集成测试覆盖 19 条财务记录、2 条结构化错误、幂等重跑、源库不变和 `commerce/reconcile.Wallets` 零差异。当前旧表迁移缺口已关闭，约 8 GB 完整副本与历史 `order_pay` 盘点仍归 M6，M4 综合退出审计仍未完成。
- 新增读者活跃度与运营概览闭环（迁移 `00055`、`GET /dashboard/overview`、GVA `Dashboard`、`reader-activity` 迁移阶段）：登录、注册会话和所有已认证 Reader 请求按 `Asia/Kuala_Lumpur` 自然日尽力记录，数据库失败仅记录结构化日志且不改变 Reader 契约；复合主键保留每日首次活跃时间。管理首页展示累计/周期新增、今日/昨日及 7/30 天去重活跃和固定 30 天零填充趋势，刷新失败保留旧数据。空 PostgreSQL 首次迁移 `applied=55`、重跑 `applied=0`；真实 PostgreSQL 覆盖午夜边界、每日去重、首次时间、统计起始边界和趋势顺序，真实只读 MySQL 8.4 → PostgreSQL 17 覆盖 3 条活动记录、1 条缺失读者结构化错误、大 ID、复合游标和幂等重跑。Playwright 在桌面及 `390x844` 视口确认指标和趋势图正常，移动端主体 `clientWidth=374`、`scrollWidth=374` 无横向溢出，Canvas `624x720` 抽样含 1205 个非透明像素，控制台为 0 error/0 warning。
- 恢复旧系统两阶段模拟充值闭环（迁移 `00056`、`POST /reader/orders/mockRecharge`、`POST /reader/orders/:id/confirmMockRecharge`）：管理端先按字符串读者 ID、充值币数量和一次性请求 ID 幂等创建 `pending` 订单，确认时锁定订单，在同一 PostgreSQL 事务写入不可变充值流水并转为 `paid`，重复创建/确认均返回原业务结果。首充成功会按固定旧规则以 `invite_reward:<inviteeId>:first_recharge` 唯一键写入邀请奖励事实并向邀请人发放一次 100 bonus；模拟充值、EPUSDT 成功回调和人工补单共用同一事务函数，因此跨入口也不会重复发奖。读者邀请面板恢复展示 `firstRechargeRewardCoin=100`，总奖励纳入首充流水。空 PostgreSQL 首次迁移 `applied=56`、重跑 `applied=0`；真实 PostgreSQL 验证重复创建后订单为 1、重复确认后充值流水为 1、首充奖励记录/奖励流水各 1，受邀读者充值余额 200、邀请人奖励余额 100；另以“先人工补单、再模拟充值确认”验证跨入口仅产生 1 条首充奖励记录和 1 条奖励流水，邀请人奖励余额仍为 100；定向 Go test、迁移清单/秘密规则、ESLint 和管理前端生产构建通过。
- 补齐邀请码编辑与类型保护（迁移 `00057`、`PUT /reader/inviteCodes/:id`）：人工码可编辑邀请码、状态、最大使用次数、过期时间和备注，最大次数不能低于已使用次数；读者自动码只允许编辑状态与备注且禁止删除。真实 PostgreSQL 集成测试覆盖编辑持久化、自动码保护和非法最大次数事务回滚；空库迁移及重跑幂等通过。Playwright 完成人工码创建/编辑/删除和自动码编辑保护闭环，桌面与 `390x844` 移动视口均通过，移动弹窗左右各保留 12px 且页面无横向溢出。
...
- 支付渠道安全 CRUD 已完成（迁移 `00073`）：管理后台支持单一 EPUSDT 渠道新建、脱敏查看、编辑、启停、连通性检查和安全归档；PID/Secret 使用全项目唯一的 `MOONBOOK_APP_MASTER_KEY` 与 AES-256-GCM 认证加密落库，接口、访问日志和操作记录不回显明文。创建订单、回调验签、主动同步和过期窗口按操作读取 PostgreSQL 配置，保存后下一次请求即时生效；进行中订单会阻止凭据替换、停用和归档。隔离 PostgreSQL 72→73、重复执行 `applied=0`、支付渠道/充值/回调/同步集成测试、管理端合同测试和生产构建作为本批次验证门；真实 EPUSDT 支付仍归外部验收。
- 支付地址配置与 GM Pay v2 同步已完成（迁移 `00074`/`00075`）：管理员只填写 EPUSDT 和读者端两个基础地址，后端统一生成创建、回调、跳转、健康和查单地址，旧五地址字段不再作为写入或运行事实；缺少新基础地址的既有渠道会保持停用，管理页面只读预览并支持复制。主动同步改为 GM Pay v2 `GET /pay/check-status/{trade_id}`，严格校验 envelope、订单号、状态、响应大小和超时；无签名“已支付”只标记缺回调异常并提示在 GM Pay 重发回调，绝不直接写钱包流水。隔离 PostgreSQL 空库 75 个迁移、重跑 `applied=0`、支付配置与同步真实库集成测试、Commerce 全包测试、`go vet`、管理端 38 项测试、ESLint 和生产构建均通过；真实支付仍归外部验收。
...
- 新增 `POST /novel/crawl/candidates/discover` 板块候选发现入口和管理页面按钮，受控抓取启用板块（HTTP/HTTPS、来源域名、4 MiB 响应体、最多 10 页），解析 Discuz 常见帖子链接后按 `(source_id,forum_thread_id)` 幂等 upsert；重复发现真实 PostgreSQL 集成测试通过，迁移 `00041` 注册 API 权限。`DiscoveryWorker` 已在应用启动、停止和配置重载生命周期中运行，按 `auto_follow_enabled` 与 `follow_interval_minutes` 调度到期板块，每板使用专属连接持有 advisory lock，单板失败隔离；真实论坛 Cookie Secret 与外部沙箱验证仍是剩余验收项。
- 新增论坛导入任务管理 API 和页面（`GET/POST /novel/crawl/importTasks`、`GET /novel/crawl/importTasks/:id`、`POST /novel/crawl/importTasks/:id/retry`、`POST /novel/crawl/importTasks/:id/cancel`），迁移 `00039` 建立任务快照、质量统计、状态/尝试次数约束及 `platform_jobs` 队列关联；创建任务会锁定候选并写入持久化队列。取消会事务性取消任务与平台 Job、关闭活动 attempt 并将候选恢复为 pending；人工重试会重新激活原 Job、追加三次尝试预算并将候选恢复为 importing。真实 PostgreSQL 集成测试覆盖三者状态联动和尝试预算。
- 新增抓取日志查询 API 和页面（`GET /novel/crawl/fetchLogs`、`GET /novel/crawl/fetchLogs/:id`），迁移 `00040` 建立请求/解析/导入阶段日志事实表、状态约束和任务索引；导入 worker 核心支持通过 `platform_jobs` claim/lease 执行、成功/失败任务状态联动及日志写入。受控 `HTTPExecutor` 限制 HTTP/HTTPS URL、超时和总响应体大小，读取来源配置的 User-Agent、Cookie 与请求间隔；按结构化 HTML `rel=next`、`nxt/next` 类名或精确“下一页/next”文本跟随同源分页，拒绝跨源重定向/下一页、循环和超过 50 页。每页 URL、HTTP 状态、字节数、耗时和结果均写入 `novel_crawl_fetch_log`，后页失败仍保留前页证据。章节写入复用 MinIO 校验上传与 PostgreSQL 对象激活，并按书籍/章节序号幂等跳过已有章节。
- 导入 Worker 已接入应用生命周期：启动时构造受控 HTTP 执行器、MinIO 章节写入器和持久化 Job Repository，持续 claim `forum_import` 任务；执行期间按租约续期，轮询前恢复过期租约，优雅停止和配置重载均会等待 Worker 退出。真实 PostgreSQL + MinIO 集成测试验证两页导入证据、来源请求策略装载、候选最后页码、取消后不写章节、可重试失败前两次保持 pending/importing、第三次耗尽后任务/Job/候选一致 failed，以及已持久化业务失败不会终止 Worker 长循环。章节写入幂等、对象激活和重复执行不新增章节继续通过；M5 全量重启恢复演练仍待完成。
- 新增 `novel_txt_import_task`（迁移 `00042`）、`/novel/txtImports` 管理 API/页面和独立 `txt_import` 持久化 Worker；上传先写入 MinIO 并校验字节数/SHA-256，再建立 PostgreSQL 任务和平台队列，Worker 重启可从租约恢复并使用 `txt_import` 章节来源写入目标书籍。`GET /novel/txtImports/:id/preview` 仅返回前 20 章和每章前 2000 个字符；前向迁移 `00043` 幂等补齐早期已执行 `00042` 环境的预览权限。模块单元测试、迁移嵌入清单测试和前端构建通过。
- 新增 `POST /novel/txtImports/:id/file`（迁移 `00044`）失败文件修复入口和管理页面操作；仅 `failed/cancelled` 任务允许替换，修复文件先经 MinIO 大小/SHA-256 校验，再在同一 PostgreSQL 事务中替换引用、清空质量/失败统计、重置任务与平台 Job 尝试次数并重新排队。全量重启演练仍待完成。
- 新增前向迁移 `00045` 修复 `00040` 抓取日志与 `00042` TXT 导入复用菜单 ID `1718` 导致的入口缺失；TXT 导入改用独立菜单 ID `1719`，已执行旧迁移的环境无需修改历史脚本即可补齐菜单与超级管理员权限。
- 新增书籍合并管理闭环（迁移 `00046`-`00047`、`/novel/bookMerges`、`view/novel/bookMerges`）：仅成功论坛导入且未成功合并的书籍可参与，按帖子创建时间（缺失时按导入任务时间）和源章节序号生成预览，疑似重复默认保留并允许人工排除。执行时重新读取并校验源 MinIO 正文，目标多章对象全部上传校验后通过批量对象激活与目标书/章节、来源快照、章节血缘和源书转草稿在同一 PostgreSQL 事务提交；源内容变化会拒绝执行，失败上传由孤立对象回收处理。真实隔离 PostgreSQL + MinIO 测试覆盖三章预览、时间排序、重复标记、排除一章、两对象激活、源书下架、三条血缘和重复合并拒绝；对象存储集成测试覆盖批量激活成功与事务回滚。active AI 清洗结果优先策略待 AI 数据模型与任务切片接入。
- 新增 AI 配置与有序模型管理闭环（迁移 `00048`、`/novel/aiConfigs`、`view/novel/aiConfigs`）：支持 OpenAI 兼容基础地址、AUTO/STREAM/NON_STREAM 调用模式、1-50 个有序模型、连续失败阈值、当前模型、失败计数、状态版本和人工重置。模型或阈值变更会原子回到首模型并递增版本；运行时以配置/模型/版本快照记录成功失败，达到阈值后循环切换且拒绝陈旧调用覆盖新状态。PostgreSQL 不保存或回显 API Key，只保存 `MOONBOOK_AI_*_API_KEY` 环境变量引用和是否已注入状态；真实 PostgreSQL 集成测试覆盖创建、启用解析、失败切换、陈旧快照、重置、模型重排和软删除。AI HTTP 传输、本地替身和清洗/摘要/画像任务将在后续执行器切片接入。
- 新增章节清洗闭环（迁移 `00049`、`/novel/chapterClean`、`view/novel/chapterClean`）：单例配置复用启用 AI 配置和 Secret，HTTP 传输兼容 OpenAI 普通 JSON 与 SSE，逐次调用记录模型成功/失败并按阈值轮换，内容过滤拒答可切换备用 AI 配置。清洗任务通过 `platform_jobs` 持久化领取、租约心跳/过期恢复和有限重试，支持停止、续跑、连续三次失败熔断、失败后继续、最低字数、最低保留比例和超长章节原文自动成功。清洗稿写入 MinIO `chapter-clean` 不可变对象，成功采用再生成正式章节正文新版本；结果支持详情对照、人工采用、人工丢弃和失败重洗，AI 原始响应保留 7 天后由 Worker 清理，Long ID 全程字符串化。隔离 PostgreSQL 已验证 `00049` 首次应用和重跑 `applied=0`，真实配置单例生命周期、传输解析、Go test/vet、定向 ESLint 和前端生产构建通过；全量恢复演练仍待完成。
...
- 服务影响：Server酱功能仅要求后端 `server` 重新构建并重启，参数变更实时生效；管理前端复用现有页面，`reader-ui`、Redis 和 MinIO 无业务改动。该功能本身不新增数据表，但本次发布同时应用行为日志迁移 `00079`。
- 上述功能于 2026-09-14 11:23（UTC+8）由 `scripts/deploy-production.sh` 发布生产：发布前已将固定提交 `89e8055c3323264db17e278867a0cbaa40907b64` 推送至 `origin/main`；镜像为 `moonbook/server:89e8055c3323`（`sha256:425197ab2c97...`）、`moonbook/web:89e8055c3323`（`sha256:5272f7d258d8...`）和 `moonbook/reader-ui:89e8055c3323`（缓存命中，`sha256:1764382789b3...`）。版本化迁移应用 1 个版本，`moonbook_schema_version` 从 78 升至 79，`reader_operation_events` 表已存在。Server、Web、Reader、Gateway 均为 healthy、restart=0，源站 Gateway、API readiness 和 Reader health 均返回 HTTP 200；发布后 Server 的 error/fatal/panic、panic 和 HTTP 5xx 计数均为 0。旧镜像保留：`server:243df5d566f3`、`web:243df5d566f3`、`reader-ui:243df5d566f3`。Server酱仍需管理员在参数管理中保存 `serverchan.send_key` 并将 `serverchan.enabled` 设为 `true` 后才会发送；生产更新已完成，无需额外重启。本条发布记录为文档变更，不影响服务，无需重启。
