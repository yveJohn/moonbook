...
M5 的运行时长任务恢复证据已经覆盖论坛导入、TXT 导入、章节清洗、章节摘要和作品画像。当前已新增内容生产前向迁移 `00063_content_legacy_source_keys.sql`，并将论坛来源/板块、候选帖子、导入任务、抓取日志 stage 接入独立命令和 `all` 编排；这些 stage 保留旧 ID、按来源键幂等重跑，历史运行中的导入任务会被标记为中断失败且不会自动访问外部论坛，抓取日志的缺失关联会保留为 NULL 并进入错误清单，并且不把 Cookie 明文写入 PostgreSQL。其余内容生产表、对象迁移、双库集成报告和全域核对器仍未完成，因此 M5 和 M6 不能关闭。
...
`all` 已在小说章节之后组合 `novel-crawl-sources`、`novel-crawl-candidates`、`novel-crawl-import-tasks` 和 `novel-crawl-fetch-logs`；独立命令可按依赖顺序重跑每个 stage。抓取日志之外仍不存在 TXT、清洗、合并或 AI stage。
