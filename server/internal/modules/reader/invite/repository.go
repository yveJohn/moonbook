...
func (r SQLRepository) RegisterWithInvite(ctx context.Context, req Registration) (RegistrationResult, error) {
	executor := transaction.Executor(ctx, r.DB)
	if executor == r.DB {
		return RegistrationResult{}, transaction.ErrNoTransaction
	}
	var invite InviteCode
	var max sql.NullInt64
	var expires sql.NullTime
	err := executor.QueryRowContext(ctx, `SELECT id,COALESCE(inviter_reader_id,0),code,status,max_use_count,used_count,expires_at FROM reader_invite_codes WHERE code=$1 FOR UPDATE`, req.InviteCode).
		Scan(&invite.ID, &invite.InviterReaderID, &invite.Code, &invite.Status, &max, &invite.UsedCount, &expires)
	if err != nil {
		return RegistrationResult{}, ErrInviteInvalid
	}
	if max.Valid {
		v := int(max.Int64)
	...
	if _, err = executor.ExecContext(ctx, `UPDATE reader_invite_codes SET used_count=used_count+1,updated_at=now() WHERE id=$1`, invite.ID); err != nil {
		return RegistrationResult{}, err
	}
	...
	if _, err = executor.ExecContext(ctx, `INSERT INTO reader_invite_codes(id,code,inviter_reader_id,status,max_use_count,used_count) VALUES((SELECT COALESCE(MAX(id),0)+1 FROM reader_invite_codes),$1,$2,'enabled',NULL,0) ON CONFLICT (inviter_reader_id) WHERE inviter_reader_id IS NOT NULL DO NOTHING`, code, account.ID); err != nil {
		return RegistrationResult{}, err
	}
	result := RegistrationResult{Account: account}
	if invite.InviterReaderID != 0 {
		if err = executor.QueryRowContext(ctx, `INSERT INTO reader_invite_relations(inviter_reader_id,invitee_reader_id,invite_code_id) VALUES($1,$2,$3) ON CONFLICT (invitee_reader_id) DO NOTHING RETURNING id`, invite.InviterReaderID, account.ID, invite.ID).Scan(&result.RelationID); err != nil && err != sql.ErrNoRows {
			return RegistrationResult{}, err
		}
		if result.RelationID != 0 {
			result.InviterID = invite.InviterReaderID
		}
	}
	return result, nil
}
